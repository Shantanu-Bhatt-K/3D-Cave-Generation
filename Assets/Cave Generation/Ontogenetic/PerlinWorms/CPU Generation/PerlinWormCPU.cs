...
public static class PerlinWormCPU
{
    ...
    public static void MeshData_Gen(int width, int height, int depth, float scale, int seed, int octaves, float lacunarity, float persistance, ref float[,,] mesh_data, int wormCount, int wormLength, float wormRadius)
    {
        ...
        for (int i = 0; i < wormCount; i++)
        {
            ...
            {
                EditMeshData(ref mesh_data, position, wormRadius, width, height, depth);
                position += GetPerlinDirection(position, scale, seed, octaves, lacunarity, persistance, octaveOffsets) * wormRadius;
            }
        }
    }
    ...
    static Vector3 GetPerlinDirection(Vector3 position, float scale, int seed, int octaves, float lacunarity, float persistance, Vector3[] octaveOffsets)
    {
        ...
        for (int l = 0; l < octaves; l++)
        {
            ...
            angleX += noiseX * amplitude * Mathf.PI * 2;
            angleY += noiseY * amplitude * Mathf.PI * 2;
            angleZ += noiseZ * amplitude * Mathf.PI * 2;

            amplitude *= persistance;
            frequency *= lacunarity;
        }

        return new Vector3(Mathf.Cos(angleX), Mathf.Sin(angleY), Mathf.Sin(angleZ)).normalized;
    }
