...
public static class Perlin_3D_GPU
{
    ...
    public static float[,,] MeshDataGen(int width, int height, int depth, float scale, int seed, int octaves, float lacunarity, float persistance, ComputeShader computeShader)
    {
        System.Diagnostics.Stopwatch st = new System.Diagnostics.Stopwatch();
        st.Start();
        Vector3[] octaveOffsets = new Vector3[octaves];
        System.Random random = new System.Random(seed);
        ComputeBuffer meshDataBuffer = new ComputeBuffer(width*height*depth, sizeof(float));
        ComputeBuffer pBuffer = new ComputeBuffer(512, sizeof(int));
        ComputeBuffer octaveOffsetBuffer = new ComputeBuffer(octaves, sizeof(float) * 3);
        ...
        computeShader.SetBuffer(0, "Result", meshDataBuffer);
        computeShader.SetBuffer(0, "p", pBuffer);
        computeShader.SetBuffer(0, "octaveOffsets", octaveOffsetBuffer);
        ...
        computeShader.Dispatch(0, Mathf.CeilToInt(width / 8.0f), Mathf.CeilToInt(height / 8.0f), Mathf.CeilToInt(depth / 8.0f));
        float[] meshData = new float[width*height*depth];
        meshDataBuffer.GetData(meshData);
        ...
        for (int k = 0; k < depth; k++)
        {
            for (int j = 0; j < height; j++)
            {
                ...
                {
                    result[i, j, k] = meshData[i + width * (j + height * k)];
                }
            }
        }
        st.Stop();
        UnityEngine.Debug.Log(string.Format("GPU 3D Perlin took {0} ms to complete", st.ElapsedMilliseconds));
        return result;
    }
}
